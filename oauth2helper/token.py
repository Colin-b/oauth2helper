...
def decode(jwt_token: str) -> (dict, dict):
    ...
    if jwt_token.count('.') < 2:
        raise InvalidTokenError('Invalid JWT Token (header, body and signature must be separated by dots).')
...
def _request_x5c(json_body: dict, key_identifier: str) -> str:
    ...
    if identity_supplier_service is None:
        raise InvalidTokenError('Identity supplier service (iss) cannot be found in JSON body.')
    ...
    raise InvalidTokenError(f'{key_identifier} is not a valid key identifier. Valid ones are {[key["kid"] for key in keys]}.')
