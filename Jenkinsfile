    ...
    stages {
        stage('Build') {
            steps {
                ...
                cd test
                nosetests . --exe --with-doctest --with-xunit --xunit-file python_unittest_out.xml --with-coverage --cover-erase --cover-package=../oauth2helper/. --cover-min-percentage=84 --cover-html
                coverage xml
                """
            }
        }
