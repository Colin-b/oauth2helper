...
pipeline {
    ...
    stages {
        ...
        stage('Test') {
            steps {
                sh """
                nosetests test --exe --with-doctest --with-xunit --xunit-file test-results.xml --with-coverage --cover-erase --cover-package=oauth2helper/. --cover-min-percentage=83 --cover-html
                """
            }
        }
