...
class TestJwtMethods(unittest.TestCase):
    ...
    def test_rsa_discovery(self):
        ...
        for key in keys_json['keys']:
            if key['kid'] == kid:
                self.assertEqual(key['x5c'][0], self.cer_in.decode('unicode_escape'))
                return
